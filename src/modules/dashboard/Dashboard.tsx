      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-[0_2px_12px_-4px_rgba(0,0,0,0.06)] border border-slate-100 lg:col-span-1 flex flex-col order-2 lg:order-1">
          ...
          <div className="h-[250px] md:h-[280px] w-full mt-auto">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={mockData.chartData} layout="vertical" margin={{ top: 0, right: 20, left: 10, bottom: 0 }}>
        ...
        <div className="bg-white p-0 rounded-2xl shadow-[0_2px_12px_-4px_rgba(0,0,0,0.06)] border border-slate-100 lg:col-span-2 overflow-hidden flex flex-col order-1 lg:order-2">
          <div className="p-4 md:p-6 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between bg-white text-brand-navy gap-3">
            ...
            <button className="text-brand-accent text-sm font-bold px-4 py-2 hover:bg-brand-accent/5 rounded-lg transition-colors border border-transparent hover:border-brand-accent/20 w-fit">View all priority</button>
          </div>
            ...
            <table className="w-full text-left text-sm text-slate-600 min-w-[600px]">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {mockData.recentJobs.map(job => (
                  <tr key={job.id} className="hover:bg-slate-50/80 transition-colors cursor-pointer group leading-relaxed">
                    ...
                    <td className="px-6 py-4">
                      <p className="font-bold text-slate-800 line-clamp-1">{job.client}</p>
                      <p className="text-xs text-slate-500 mt-0.5 font-medium line-clamp-1">{job.asset}</p>
                    </td>
                    <td className="px-6 py-4">
